...
export const submitContactForm = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const formType = String(data.get("formType") ?? "mortgage");
    ...
    for (const key of ["name", "phone", "email", "message"]) {
      ...
      if (typeof value === "string" && value.trim()) {
        ...
        rows.push(
          `<tr><td style="padding:6px 12px;color:#6b7280;font-weight:600;">${label}</td><td style="padding:6px 12px;">${escapeHtml(value)}</td></tr>`,
        );
      }
    }
    ...
    const fromEmail =
      ...
      "אתר אורן סממה <onboarding@resend.dev>";
